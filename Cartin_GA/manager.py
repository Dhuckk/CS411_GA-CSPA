...
class Manager(object):
    ...
    showIndex = True
    n_points = n
    algorithms = ["Brute Force", "Lexicographic Order", "Genetic Algorithm",
                  "Ant Colony ACS", "Ant Colony Elitist", "Ant Colony Max-Min"]

    genetic = Genetic([sample(list(range(n)), n)
                      for i in range(populationSize)], populationSize)
    ...
    def ResetGenetic(self):
        self.genetic = Genetic([sample(list(range(n)), n)
                               for i in range(populationSize)], populationSize)
    ...
    def UpdateCaption(self):
        ...
        pygame.display.set_caption(
            "Traveling Salesman using Genetic Algorithm - Fps : {}".format(frameRate))
    ...
    def RandomPoints(self):
        self.Points = [Point(randint(offset, width-offset),
                             randint(offset, height-offset)) for i in range(self.n_points)]
    ...
    def ShowText(self, selectedIndex, started=True):
        ...
        textSurface1 = textFont.render(
            "Best distance : " + str(round(self.recordDistance, 2)), False, textColor)
        textSurface2 = textFont.render(
            self.algorithms[selectedIndex], False, textColor)
        textSurface3 = textFont2.render(
        ...
        if len(self.OptimalRoutes) > 0:
            for n in range(self.n_points):
                ...
                pygame.draw.line(self.screen, self.Highlight,
                                 (self.OptimalRoutes[n].x,
                                  self.OptimalRoutes[n].y),
                                 (self.OptimalRoutes[_i].x,
                                  self.OptimalRoutes[_i].y),
                                 self.LineThickness)
    ...
    def DrawLines(self, drawCurrent=False):
        if drawCurrent == True:
            for i, point in enumerate(self.currentList):
                ...
                pygame.draw.line(self.screen, self.Gray, (point.x, point.y),
                                 (self.currentList[_i].x, self.currentList[_i].y), 1)
        else:
            for i, point in enumerate(self.Points):
                _i = (i+1) % len(self.Points)
                pygame.draw.line(self.screen, self.Gray, (point.x, point.y),
                                 (self.Points[_i].x, self.Points[_i].y), 1)
